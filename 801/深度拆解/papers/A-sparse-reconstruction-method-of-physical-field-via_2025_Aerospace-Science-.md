...
> 自动分析说明：以下基于 `801/文本/txt/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.txt` 的章节标题抽取与标题关键词判断生成；PDF 抽取可能漏掉跨行小标题，最终章节名仍建议结合原 PDF 目录和版面核查。
...
> 本区块由 `tools/upgrade_801_deep_analysis.py` 基于 `801/文本/txt/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.txt` 与 `801/文本/metadata/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.json` 重新抽取生成；用于修正旧报告中章节未全、引文缺失、摘要/结论未完整摘录的问题。双栏 PDF 抽取仍可能存在断行，引用和公式编号以 PDF 版面为最终准绳。

### 20.1 严格章节树（按 PDF/metadata TOC）

- L2 p.1: 1 Introduction （背景/领域定位）
- L2 p.3: 2 Methods （方法/模型）
  - L3 p.3: 2.1 Sparse reconstruction method via multi-source sensors （方法/模型）
  - L3 p.4: 2.2 Minimum number of the multi-source sensors （对象/问题/模块）
    - L4 p.4: 2.2.1 Determine the rank of physical field data （对象/问题/模块）
    - L4 p.4: 2.2.2 Determine the minimum number of sensors （对象/问题/模块）
  - L3 p.5: 2.3 Sparse reconstruction model using rom-based surrogate （方法/模型）
  - L3 p.6: 2.4 Sensor location optimization using de algorithm （对象/问题/模块）
- L2 p.6: 3 Test case （结果/讨论/验证）
  - L3 p.6: 3.1 Description of physical model （方法/模型）
  - L3 p.6: 3.2 Modeling parameters analysis （方法/模型）
  - L3 p.8: 3.3 Reconstruction results （结果/讨论/验证）
  - L3 p.10: 3.4 Analysis of sensor noise （对象/问题/模块）
- L2 p.11: 4 Conclusions （结论）
...
- L2 p.12: Appendix. Additional information of test cases （结果/讨论/验证）
...
| 2 Methods | 3 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.1 Sparse reconstruction method via multi-source sensors | 3 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.2 Minimum number of the multi-source sensors | 4 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.2.1 Determine the rank of physical field data | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.2.2 Determine the minimum number of sensors | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.3 Sparse reconstruction model using rom-based surrogate | 5 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.4 Sensor location optimization using de algorithm | 6 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3 Test case | 6 | 2 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.1 Description of physical model | 6 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.2 Modeling parameters analysis | 6 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.3 Reconstruction results | 8 | 3 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.4 Analysis of sensor noise | 10 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4 Conclusions | 11 | 2 | 结论 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
| Appendix. Additional information of test cases | 12 | 2 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
> 公开库不直接展示完整英文摘要原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。
...
结论章节识别：4 Conclusions；状态：独立结论章节

> 公开库不直接展示完整英文结论原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.json` 的 `conclusion` 字段，以及 `801/深度拆解/local_full_reports/A-sparse-reconstruction-method-of-physical-field-via_2025_Aerospace-Science-.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。

### 20.6 结论中文翻译

> 本研究提出了一种基于多源传感器测量的多个物理场数据的稀疏重建方法。该方法使用 POD-RBFNN 通过多源传感器测量构建高效的稀疏重建模型。它结合了传感器位置优化的DE算法，提供最佳的多源传感器位置和相应的稀疏重建模型。通过两个再入舱物理场的稀疏重建对该方法进行了验证。 The key conclusions are summarized below. 1.所提出的方法利用多源传感器测量来构建稀疏重建模型，显着减少传感器的数量。它根据系统可观测性确定确保高重建精度所需的最小传感器数量。 2. 反复优化表明该方法具有很强的鲁棒性，体现在重构误差方差小、传感器类型和位置一致、最佳传感器位置分布具有物理意义。 3. 与单源重建方法相比，该方法擅长用有限数量的传感器精确重建多个物理场，并且在更多传感器可用时实现更好的性能。 4.验证了该方法在测量噪声下的有效性。
> 
> 即使在不同的噪声水平下，多源传感器的最小数量也保持稳定，表现出与噪声无关的性能。
...
- 引用簇样例：[7,8], [9], [10], [11], [4,5], [12,13], [6], [27], [14], [15], [28,29], [5]
...
- [9] Z. Bai, T. Wimalajeewa, Z. Berger, G. Wang, M. Glauser, P.K. Varshney, Lowdimensional approach for reconstruction of airfoil data via compressive sensing, AIAA J. 53 (4) (2015) 920–933, https://doi.org/10.2514/1.J053287.

### 20.9 常用词、词类、语态与时态

- 高频词：reconstruction(158)；sparse(118)；sensor(80)；field(77)；sensors(76)；physical(66)；number(63)；data(59)；multi-source(54)；heat(46)；flux(45)；pressure(37)；rank(37)；fig(36)；error(29)；flow(28)；accuracy(27)；fields(26)；locations(26)；measurements(25)
- 高频名词化/学术名词：reconstruction(158)；pressure(37)；optimization(25)；function(17)；location(16)；science(14)；section(11)；information(7)；struction(7)；performance(6)；measurement(5)；observability(5)；proportion(4)；turbulence(4)；ensure(4)
- 高频学术动词：compared(5)；validated(5)；optimized(3)；presented(3)；demonstrate(3)；developed(2)；predicted(2)；indicate(1)；optimize(1)；derived(1)；propose(1)
- 高频形容词：physical(66)；total(9)；table(9)；neural(8)；objective(6)；traditional(5)；measurement(5)；schematic(5)；available(4)；theoretical(4)；high-dimensional(4)；computational(4)；dominant(4)；consistent(4)；various(3)
- 高频副词：respectively(9)；only(5)；generally(4)；directly(4)；significantly(4)；additionally(4)；subsequently(4)；accurately(4)；similarly(3)；effectively(3)；specifically(3)；theoretically(3)；relatively(3)；rapidly(3)；firstly(2)
- 高频二词短语：sparse reconstruction(96)；physical field(43)；heat flux(42)；field data(42)；number sensors(31)；multi-source sensor(23)；sensor locations(21)；pressure heat(20)；multi-source sensors(19)；minimum number(19)；sensor measurements(18)；flow conditions(16)
- 高频三词短语：physical field data(27)；pressure heat flux(18)；aerospace science technology(13)；page aerospace science(12)；heat flux fields(12)；sensor location optimization(12)；heat flux field(11)；number multi-source sensors(9)；minimum number sensors(9)；minimum number multi-source(8)；multi-source sensor measurements(8)；science technology fig(8)
- 被动语态估计：86；`we + 动作动词` 主动句估计：1
- 一般现在时线索：134；一般过去时线索：298；现在完成时线索：3；情态动词线索：43

章节词频：

- Abstract: sensors(10)；reconstruction(8)；physical(6)；number(6)；field(4)；sparse(4)；accuracy(4)；fields(3)
- Introduction: reconstruction(49)；sparse(45)；physical(24)；sensor(23)；field(21)；data(21)；sensors(20)；number(20)
- Conclusion: reconstruction(8)；sensor(7)；sparse(5)；sensors(5)；multi-source(4)；number(4)；physical(3)；measurements(3)

### 20.10 句型库扩充（每类多句）

#### 背景句
- 原句/结构：However, CS requires optimization to solve basis coefficients and is inherently NP-hard, posing challenges for ensuring sparse reconstruction accuracy.
...
- 原句/结构：Measuring distributed loads of physical fields like force and heat on surface is essential for aerodynamic design, integrated thermal protec tion system analysis, health monitoring, and flight control of flight vehicle [1–3].
...
- 原句/结构：However, CS requires optimization to solve basis coefficients and is inherently NP-hard, posing challenges for ensuring sparse reconstruction accuracy.
  可迁移模板：However, METHOD requires optimization to solve basis coefficients and is inherently METHOD-hard, posing challenges for ensuring sparse reconstruction accuracy.
#### Gap句
- 原句/结构：Originating in signal processing, CS leverages signal sparsity to recon struct complete signals from limited observations and has been widely used for sparse reconstruction of high-dimensional physical field data [7,8].
...
- 原句/结构：However, CS requires optimization to solve basis coefficients and is inherently NP-hard, posing challenges for ensuring sparse reconstruction accuracy.
  可迁移模板：However, METHOD requires optimization to solve basis coefficients and is inherently METHOD-hard, posing challenges for ensuring sparse reconstruction accuracy.
- 原句/结构：However, this improvement also led to increased model complexity and reduced computational efficiency.
  可迁移模板：However, this improvement also led to increased model complexity and reduced computational efficiency.
#### 方法句
- 原句/结构：Given that different physical fields under the same flow conditions are correlated, a sparse reconstruction method utilizing information from different types of sensors is proposed to reduce the number of sensors and improve reconstruction accuracy.
...
- 原句/结构：Meanwhile, a theoretical analysis is conducted to determine the minimum number of multi-source sensors required by the proposed method to achieve target reconstruction accuracy.
...
- 原句/结构：The method is achieved by three steps.
  可迁移模板：The method is achieved by three steps.
#### 结果句
- 原句/结构：Meanwhile, a theoretical analysis is conducted to determine the minimum number of multi-source sensors required by the proposed method to achieve target reconstruction accuracy.
  可迁移模板：Meanwhile, a theoretical analysis is conducted to determine the minimum number of multi-source sensors required by the proposed method to achieve target reconstruction accuracy.
- 原句/结构：The method is achieved by three steps.
  可迁移模板：The method is achieved by three steps.
- 原句/结构：The results indicate that, compared with traditional single-source reconstruction method, the proposed method can decrease the total sensor number by about 50% with the same accuracy level, and with the same total number of sensors, the sparse reconstruction error can be reduced by 16% to 99%.
...
#### 贡献句
- 原句/结构：The physical field reconstruction depending on sensors installed sparsely on the surface can provide more valuable and detailed information for design, control, and maintenance of flight vehicles.
...
- 原句/结构：Aerospace Science and Technology 167 (2025) 110685 Contents lists available at ScienceDirect Aerospace Science and Technology journal homepage: www.elsevier.com/locate/aescte The physical field reconstruction depending on sensors installed sparsely on the surface can provide more valuable and detailed information for design, control, and maintenance of flight vehicles.
  可迁移模板：Aerospace Science and Technology X(X) XContents lists available at ScienceDirect Aerospace Science and Technology journal homepage: www.elsevier.com/locate/aescte The physical field reconstruction depending on sensors installed sparsely on the surface can provide more valuable and detailed information for design, control, and maintenance of flight vehicles.
- 原句/结构：In summary, while previous research on sparse reconstruction models and sensor location optimization is well-established, it mainly focuses on single-source field data reconstruction, with limited explo ration of multi-source field data.
  可迁移模板：In summary, while previous research on sparse reconstruction models and sensor location optimization is well-established, it mainly focuses on single-source field data reconstruction, with limited explo ration of multi-source field data.
#### 限制/边界句
- 原句/结构：Originating in signal processing, CS leverages signal sparsity to recon struct complete signals from limited observations and has been widely used for sparse reconstruction of high-dimensional physical field data [7,8].
  可迁移模板：Originating in signal processing, METHOD leverages signal sparsity to recon struct complete signals from limited observations and has been widely used for sparse reconstruction of high-dimensional physical field data [X,X].
- 原句/结构：However, the limited sensor locations on the fight vehicle surface poses challenges in obtaining complete field data.
...
- 原句/结构：To address this issue, researchers have developed sparse reconstruction methods to reconstruct physical filed data from limited sensor measurements.

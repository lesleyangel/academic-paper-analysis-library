...
- 正文词频只统计正文主章节：1 Introduction, 2 Design of the novel solar radiant heating test, 4 Results and discussion, 5 Conclusions。
- 排除范围：摘要、References、图题/表题、页眉页脚、版权信息、期刊首页信息、旧分析文字。
- 正文切分告警：
- 3 Formulations of the coupled temperature-diffusion-expansion problem: 正文抽取偏短
...
- 摘要抽取状态：成功。
- 完整英文摘要原文不在公开报告正文中展开；本地完整摘录见 `801/深度拆解/extracted_evidence/A-solar-radiant-heating-apparatus-for-measuring-the-thermal_2016_Applied-The.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/A-solar-radiant-heating-apparatus-for-measuring-the-thermal_2016_Applied-The.md`。
...
- 已有研究不足/GAP：However, the thermal protection material experiences an irreversible physico-chemical process of ablation under elevated and high temperatures [4].
...
- L2 p.2: 2 Design of the novel solar radiant heating test（对象/模块/过渡章节）
  - L3 p.2: 2.1 Apparatus（对象/模块/过渡章节）
  - L3 p.3: 2.2 Experimental procedure（结果/验证/讨论）
- L2 p.3: 3 Formulations of the coupled temperature-diffusion-expansion problem（方法/模型/算法）
  - L3 p.3: 3.1 Energy conservation equation（对象/模块/过渡章节）
  - L3 p.4: 3.2 Mass transforming relationship（对象/模块/过渡章节）
  - L3 p.4: 3.3 Formulations for thermochemical expansion（方法/模型/算法）
  - L3 p.4: 3.4 Gas diffusion equation（对象/模块/过渡章节）
- L2 p.5: 4 Results and discussion（结果/验证/讨论）
- L2 p.6: 5 Conclusions（结论/贡献回收）
...
| 2 Design of the novel solar radiant heating test | 2 | 2 | 对象/模块/过渡章节 |
| 2.1 Apparatus | 2 | 3 | 对象/模块/过渡章节 |
| 2.2 Experimental procedure | 3 | 3 | 结果/验证/讨论 |
| 3 Formulations of the coupled temperature-diffusion-expansion problem | 3 | 2 | 方法/模型/算法 |
| 3.1 Energy conservation equation | 3 | 3 | 对象/模块/过渡章节 |
| 3.2 Mass transforming relationship | 4 | 3 | 对象/模块/过渡章节 |
| 3.3 Formulations for thermochemical expansion | 4 | 3 | 方法/模型/算法 |
| 3.4 Gas diffusion equation | 4 | 3 | 对象/模块/过渡章节 |
| 4 Results and discussion | 5 | 2 | 结果/验证/讨论 |
| 5 Conclusions | 6 | 2 | 结论/贡献回收 |
...
- 正文统计字符数：22564
- 高频词：thermal(48)；heating(40)；heat(37)；material(26)；temperature(24)；high(23)；test(21)；composites(21)；specimen(20)；phenolic(20)；materials(19)；pyrolysis(19)；decomposition(19)；flux(17)；apparatus(17)；radiant(17)；silica(17)；gas(17)；model(16)；mass(16)
- 高频名词化/学术名词：temperature(24)；decomposition(19)；protection(13)；expansion(13)；density(13)；thickness(9)；equation(9)；exposure(6)；prism(5)；ablation(4)；diffusion(4)；radiation(4)；direction(4)；reaction(4)；high-temperature(3)
- 高频学术动词：developed(8)；presented(7)；predicted(4)；provide(3)；validate(2)；estimate(2)；evaluate(2)；estimated(2)；predict(1)；evaluated(1)；provided(1)；analyzed(1)；construct(1)
- 高频形容词：thermal(48)；material(26)；phenolic(20)；radiant(17)；boundary(11)；experimental(11)；thermochemical(6)；initial(6)；mathematical(5)；various(5)；porous(4)；evident(4)；electric(3)；differential(3)；numerical(3)
- 高频副词：simultaneously(3)；theoretically(3)；generally(3)；firstly(3)；vertically(3)；roughly(2)；perfectly(1)；easily(1)；mainly(1)；widely(1)；supply(1)；secondly(1)；thermally(1)；additionally(1)；subsequently(1)
- 高频二词短语：thermal protection(13)；heat flux(13)；silica phenolic(13)；frp composites(10)；radiant heating(9)；phenolic composites(9)；heat fluxes(8)；char layer(8)；pyrolysis zone(8)；protection materials(7)；virgin material(7)；high temperatures(6)
- 高频三词短语：silica phenolic composites(9)；thermal protection materials(7)；char layer pyrolysis(5)；layer pyrolysis zone(5)；pyrolysis zone virgin(5)；zone virgin material(5)；thermal protection material(4)；high heat flux(4)；solar radiant heating(4)；composites high temperatures(4)；silica phenolic composite(4)；heat affected zone(3)
- 被动语态估计：81；`we + 动作动词` 主动句估计：1
- 一般现在时线索：95；一般过去时线索：218；现在完成时线索：0；情态动词线索：27

分章节正文词频：

- 1 Introduction: thermal(17)；heating(13)；high(10)；protection(9)；heat(9)；materials(7)；test(6)；model(6)
- 2 Design of the novel solar radiant heating test: thermal(20)；heat(19)；heating(17)；decomposition(14)；test(13)；specimen(13)；temperature(12)；material(12)
- 4 Results and discussion: surface(9)；density(9)；heating(8)；char(8)；pyrolysis(8)；zone(8)；material(8)；layer(7)
- 5 Conclusions: heat(5)；thermal(5)；composites(4)；flux(3)；temperature(3)；model(3)；mass(3)；silica(3)
...
#### Gap/转折句
- 原句：However, the thermal protection material experiences an irreversible physico-chemical process of ablation under elevated and high temperatures [4].
...
- 原句：However, fabrication and operating costs of this equipment system is higher because of a number of sophisticated components, also adjustment and calibration of the heat flux parameters is difficult [11,12].
...
#### 方法提出句
- 原句：These experimental data are suitable for use in verification of numerical models.
  可迁移模板：These experimental data are suitable for use in verification of numerical models.
- 原句：A mathematical model for the coupled temperature-diffusionexpansion problem of porous materials was proposed.
  可迁移模板：A mathematical model for the coupled temperature-diffusionexpansion problem of porous materials was proposed.
- 原句：These experimental data are suitable for use in verification of numerical models.
  可迁移模板：These experimental data are suitable for use in verification of numerical models.
- 原句：A mathematical model for the coupled temperature-diffusionexpansion problem of porous materials was proposed.
  可迁移模板：A mathematical model for the coupled temperature-diffusionexpansion problem of porous materials was proposed.
- 原句：Moreover, many mathematical models for predicting the thermal behavior of these materials also require accurate experimental data.
  可迁移模板：Moreover, many mathematical models for predicting the thermal behavior of these materials also require accurate experimental data.
#### 结果呈现句
- 原句：Although oxyacetylene torch test could be utilized to study the high temperature oxidation performance and ablation behavior of ablative materials, the oxyacetylene flame may alter the flowfield around the ablator, which, in turn, affects the measured results [13,14].
...
#### 贡献/增量句
- 原句：To provide insight into this complex high-temperature behavior, a controlled heating environment with well defined boundary conditions which can simulate the actual operating conditions of the thermal protection material is necessary.
...
- 原句：The temperature measurements were conducted to provide the experimental data necessary to evaluate the accuracy of the numerical model.
...
- 原句：Although oxyacetylene torch test could be utilized to study the high temperature oxidation performance and ablation behavior of ablative materials, the oxyacetylene flame may alter the flowfield around the ablator, which, in turn, affects the measured results [13,14].
  可迁移模板：Although oxyacetylene torch test could be utilized to study the high temperature oxidation performance and ablation behavior of ablative materials, the oxyacetylene flame may alter the flowfield around the ablator, which, in turn, affects the measured results [X,X].
- 原句：Besides, the additional electric or magnetic fields may affect the microstructures of the experimental materials.
  可迁移模板：Besides, the additional electric or magnetic fields may affect the microstructures of the experimental materials.
- 原句：Therefore, the interferences mentioned above should be avoided when we theoretically test the various responses of the thermal protection materials with temperature, and the radiant heating is thought to be the desired heating method.
...
- 引文簇样例：[4], [5], [29], [9,10], [11,12], [13,14], [15,16], [18], [22], [23], [24], [25]

...
- 正文词频只统计正文主章节：1 Introduction, 2 Problem formulation, 3 Methodology, 4 Simulations and discussion, 5 Conclusions。
...
- 摘要抽取状态：成功。
- 完整英文摘要原文不在公开报告正文中展开；本地完整摘录见 `801/深度拆解/extracted_evidence/A-three-stage-sequential-convex-programming-approa_2024_Aerospace-Science-an.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/A-three-stage-sequential-convex-programming-approa_2024_Aerospace-Science-an.md`。
...
- 已有研究不足/GAP：Indirect methods [9] have great advantages in solving simple problems, but they are difficult to imple ment when the control system is a little complicated.
...
- L2 p.2: 3 Methodology（方法/模型/算法）
  - L3 p.2: 3.1 Direct collocation method with hp RPD（方法/模型/算法）
  - L3 p.3: 3.2 Fundamental of conventional SCP approach（方法/模型/算法）
  - L3 p.5: 3.3 Improved hp-Adaptive mesh refinement algorithm（方法/模型/算法）
  - L3 p.6: 3.4 The three-stage scp approach（方法/模型/算法）
- L2 p.7: 4 Simulations and discussion（结果/验证/讨论）
  - L3 p.7: 4.1 Reentry trajectory optimization with dual-channel control（对象/模块/过渡章节）
  - L3 p.9: 4.2 Ascent trajectory optimization of launch vehicle（对象/模块/过渡章节）
- L2 p.11: 5 Conclusions（结论/贡献回收）
- L2 p.11: Declaration of competing interest（尾部材料）
- L2 p.11: Data availability（尾部材料）
- L2 p.11: Acknowledgments（尾部材料）
- L2 p.12: Appendix A Description of Eq. (28)（问题定义）
- L2 p.12: Appendix B Details of Reentry Example（尾部材料）
- L2 p.14: Appendix C Details of Ascent Example（尾部材料）
...
| 3 Methodology | 2 | 2 | 方法/模型/算法 |
| 3.1 Direct collocation method with hp RPD | 2 | 3 | 方法/模型/算法 |
| 3.2 Fundamental of conventional SCP approach | 3 | 3 | 方法/模型/算法 |
| 3.3 Improved hp-Adaptive mesh refinement algorithm | 5 | 3 | 方法/模型/算法 |
| 3.4 The three-stage scp approach | 6 | 3 | 方法/模型/算法 |
| 4 Simulations and discussion | 7 | 2 | 结果/验证/讨论 |
| 4.1 Reentry trajectory optimization with dual-channel control | 7 | 3 | 对象/模块/过渡章节 |
| 4.2 Ascent trajectory optimization of launch vehicle | 9 | 3 | 对象/模块/过渡章节 |
| 5 Conclusions | 11 | 2 | 结论/贡献回收 |
| Declaration of competing interest | 11 | 2 | 尾部材料 |
| Data availability | 11 | 2 | 尾部材料 |
| Acknowledgments | 11 | 2 | 尾部材料 |
| Appendix A Description of Eq. (28) | 12 | 2 | 问题定义 |
| Appendix B Details of Reentry Example | 12 | 2 | 尾部材料 |
| Appendix C Details of Ascent Example | 14 | 2 | 尾部材料 |
...
- 正文统计字符数：18716
- 高频词：problem(27)；optimization(25)；trajectory(24)；scp(23)；control(22)；constraints(15)；points(15)；approach(14)；methods(13)；discretization(13)；function(13)；method(12)；initial(11)；approaches(11)；problems(10)；direct(9)；system(9)；solution(8)；constraint(8)；stage(8)
- 高频名词化/学术名词：optimization(25)；discretization(13)；function(13)；solution(8)；collocation(7)；guidance(6)；iteration(6)；convergence(6)；transcription(6)；section(5)；relaxation(4)；refinement(4)；convexification(4)；dynamics(4)；interpolation(4)
- 高频学术动词：compared(3)；indicate(2)；evaluate(1)
- 高频形容词：initial(11)；objective(8)；terminal(8)；general(6)；optimal(5)；pseudospectral(5)；numerical(5)；original(5)；dynamic(5)；boundary(5)；refinement(4)；iterative(4)；polynomial(4)；efficient(3)；sequential(3)
- 高频副词：directly(4)；typically(3)；finally(3)；greatly(2)；successfully(2)；explicitly(2)；firstly(2)；gradually(1)；recently(1)；quickly(1)；usually(1)；mostly(1)；sufficiently(1)；early(1)；undoubtedly(1)
- 高频二词短语：trajectory optimization(18)；scp approaches(11)；scp approach(6)；optimization problems(6)；optimal control(5)；direct collocation(5)；original problem(5)；initial terminal(5)；knot points(5)；constraint relaxation(4)；mesh refinement(4)；reentry trajectory(4)
- 高频三词短语：trajectory optimization problems(5)；reentry trajectory optimization(4)；optimal control problem(3)；sequential convex programming(3)；direct collocation methods(3)；trajectory optimization problem(3)；system dynamics constraints(3)；trajectory optimi zation(2)；three-stage scp approach(2)；radau pseudospectral discretization(2)；stage mesh refinement(2)；conventional scp approaches(2)
- 被动语态估计：46；`we + 动作动词` 主动句估计：0
- 一般现在时线索：86；一般过去时线索：106；现在完成时线索：1；情态动词线索：15

分章节正文词频：

- 1 Introduction: scp(16)；optimization(14)；problem(14)；trajectory(13)；methods(13)；discretization(10)；control(9)；method(8)
- 2 Problem formulation: function(8)；constraints(7)；system(5)；problem(4)；trajectory(3)；terminal(3)；dynamic(3)；objective(3)
- 3 Methodology: points(13)；problem(6)；control(6)；state(5)；constraints(5)；follows(5)；polynomial(4)；knot(4)
- 4 Simulations and discussion: angle(7)；optimization(5)；control(5)；reentry(5)；trajectory(5)；example(4)；initial(4)；qmax(4)
- 5 Conclusions: approach(4)；problems(3)；scp(3)；approaches(3)；general(2)；trajectory(2)；optimization(2)；convergence(2)
...
#### Gap/转折句
- 原句：Indirect methods [9] have great advantages in solving simple problems, but they are difficult to imple ment when the control system is a little complicated.
...
- 原句：However, it is typically difficult to solve an NLP prob lem, and the performance of the direct method is mostly governed by the nonlinear programming solver.
...
- 原句：Over the last few years, many scholars proposed using sequence convex optimization (SCP) approaches to solve trajectory optimization problems [11,12].
  可迁移模板：Over the last few years, many scholars proposed using sequence convex optimization (METHOD) approaches to solve trajectory optimization problems [X,X].
- 原句：The remainder of this paper is organized as follows.
...
- 原句：Recently, sequential convex programming (SCP) has become a potential approach in trajectory optimization because of its high efficiency.
  可迁移模板：Recently, sequential convex programming (METHOD) has become a potential approach in trajectory optimization because of its high efficiency.
...
- 原句：A dual-channel control reentry trajectory optimization and an ascent trajectory optimization are taken as examples, and the simulation results show that the proposed approach outperforms conventional SCP approaches in terms of accuracy and efficiency.
  可迁移模板：A dual-channel control reentry trajectory optimization and an ascent trajectory optimization are taken as examples, and the simulation results show that the proposed METHOD approaches in terms of accuracy and efficiency.
...
- 原句：Taking online optimization results as guidance instructions [4,5] or constructing a “guidance model” based on offline trajectory optimiza tion data [6] are the two main ways to realize advanced guidance.
...
- 原句：Taking online optimization results as guidance instructions [4,5] or constructing a “guidance model” based on offline trajectory optimiza tion data [6] are the two main ways to realize advanced guidance.
  可迁移模板：Taking online optimization results as guidance instructions [X,X] or constructing a “guidance model” based on offline trajectory optimiza tion data [X] are the two main ways to realize advanced guidance.
- 原句：A dual-channel control reentry trajectory optimization and an ascent trajectory optimization are taken as examples, and the simulation results show that the proposed approach outperforms conventional SCP approaches in terms of accuracy and efficiency. improvement of numerical optimization algorithms, it is possible to solve large-scale optimization problems quickly.
  可迁移模板：A dual-channel control reentry trajectory optimization and an ascent trajectory optimization are taken as examples, and the simulation results show that the proposed METHOD approaches in terms of accuracy and efficiency. improvement of numerical optimization algorithms, it is possible to solve large-scale optimization problems quickly.
- 原句：Simple discretization schemes and low-order numerical inte gration and differentiation were used for problem transcription in the early SCP approaches, which undoubtedly resulted in poor accuracy of results.
  可迁移模板：Simple discretization schemes and low-order numerical inte gration and differentiation were used for problem transcription in the early METHOD approaches, which undoubtedly resulted in poor accuracy of results.
- 原句：Section 4 shows numerical simulations and a dis cussion of the results, and Section 5 summarizes the main work of this paper.
  可迁移模板：Section Xshows numerical simulations and a dis cussion of the results, and Section Xsummarizes the main work of this paper.
#### 贡献/增量句
- 原句：To improve stability and discretization accuracy, a three-stage SCP approach based on the hp-adaptive Radau pseudospectral discretization is proposed in this paper.
  可迁移模板：To improve stability and discretization accuracy, a three-stage METHOD approach based on the hp-adaptive Radau pseudospectral discretization is proposed in this paper.
- 原句：To improve stability and discretization accuracy, a three-stage SCP approach based on the hp-adaptive Radau pseudospectral discretization is proposed in this paper.
...
- 原句：A dual-channel control reentry trajectory optimization and an ascent trajectory optimization are taken as examples, and the simulation results show that the proposed approach outperforms conventional SCP approaches in terms of accuracy and efficiency. improvement of numerical optimization algorithms, it is possible to solve large-scale optimization problems quickly.
  可迁移模板：A dual-channel control reentry trajectory optimization and an ascent trajectory optimization are taken as examples, and the simulation results show that the proposed METHOD approaches in terms of accuracy and efficiency. improvement of numerical optimization algorithms, it is possible to solve large-scale optimization problems quickly.
- 原句：In the second stage, adaptive mesh refinement al gorithm is used to improve the discretization accuracy.
  可迁移模板：In the second stage, adaptive mesh refinement al gorithm is used to improve the discretization accuracy.
- 原句：Based on constraint relaxation, mesh refinement, and iterative damping, the problems of poor iteration stability, large discretization error, and slow convergence speed of conventional SCP approaches are effectively improved.
  可迁移模板：Based on constraint relaxation, mesh refinement, and iterative damping, the problems of poor iteration stability, large discretization error, and slow convergence speed of conventional METHOD approaches are effectively improved.
#### 限制/边界句
...
- 原句：On the other hand, it may be more efficient to approximate nonlinear constraints using quadratic functions (which require the use of BFPS, etc., to approximate the Hessian matrix) than simple linearization.
  可迁移模板：On the other hand, it may be more efficient to approximate nonlinear constraints using quadratic functions (which require the use of METHOD, etc., to approximate the Hessian matrix) than simple linearization.
...
- 引文簇样例：[1,2], [7], [3], [10], [4,5], [6], [11,12], [7,8], [13], [14], [9], [1,19-21]

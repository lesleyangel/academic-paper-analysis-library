...
> 自动分析说明：以下基于 `801/文本/txt/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.txt` 的章节标题抽取与标题关键词判断生成；PDF 抽取可能漏掉跨行小标题，最终章节名仍建议结合原 PDF 目录和版面核查。
...
> 本区块由 `tools/upgrade_801_deep_analysis.py` 基于 `801/文本/txt/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.txt` 与 `801/文本/metadata/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.json` 重新抽取生成；用于修正旧报告中章节未全、引文缺失、摘要/结论未完整摘录的问题。双栏 PDF 抽取仍可能存在断行，引用和公式编号以 PDF 版面为最终准绳。

### 20.1 严格章节树（按 PDF/metadata TOC）

- L2 p.2: 1 Introduction （背景/领域定位）
- L2 p.3: 2 Problem description （对象/问题/模块）
- L2 p.4: 3 Methods （方法/模型）
  - L3 p.4: 3.1 Data-driven reconstruction （对象/问题/模块）
  - L3 p.5: 3.2 Tensorized Fourier neural operator （对象/问题/模块）
- L2 p.7: 4 Heat flux distribution reconstruction for a blunt cone （对象/问题/模块）
  - L3 p.7: 4.1 Physical model （方法/模型）
  - L3 p.9: 4.2 TFNO model configuration and training （方法/模型）
  - L3 p.10: 4.3 Results （结果/讨论/验证）
- L2 p.16: 5 Temperature distribution reconstruction for a C/SiC plate （对象/问题/模块）
  - L3 p.16: 5.1 Physical model （方法/模型）
  - L3 p.18: 5.2 TFNO model configuration and training （方法/模型）
  - L3 p.19: 5.3 Results （结果/讨论/验证）
- L2 p.22: 6 Conclusion （结论）
- L2 p.22: CRediT authorship contribution statement （对象/问题/模块）
- L2 p.22: Declaration of competing interest （对象/问题/模块）
- L2 p.22: Acknowledgments （对象/问题/模块）
- L2 p.22: Data availability （对象/问题/模块）
...
| 3 Methods | 4 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.1 Data-driven reconstruction | 4 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.2 Tensorized Fourier neural operator | 5 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4 Heat flux distribution reconstruction for a blunt cone | 7 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4.1 Physical model | 7 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4.2 TFNO model configuration and training | 9 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4.3 Results | 10 | 3 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 5 Temperature distribution reconstruction for a C/SiC plate | 16 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 5.1 Physical model | 16 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 5.2 TFNO model configuration and training | 18 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 5.3 Results | 19 | 3 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 6 Conclusion | 22 | 2 | 结论 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
| Acknowledgments | 22 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Data availability | 22 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
> 公开库不直接展示完整英文摘要原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。
...
> 公开库不直接展示完整英文结论原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.json` 的 `conclusion` 字段，以及 `801/深度拆解/local_full_reports/Real-time-reconstruction-of-thermal-fields-from-sp_2026_International-Journa.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。

### 20.6 结论中文翻译

> 在这项工作中，我们制定了一个基于 TFNO 的物理引导、数据驱动的反演框架，用于根据稀疏测量重建高超声速配置的完整热场。该方法集成了（i）复制掩蔽以针对任意稀疏传感器布局强化模型，（ii）离散传感器与飞行状态变量的多输入融合和钝锥体上空气动力学加热的低保真度（CFD）预测，以及（iii）简短的历史测量以恢复辐射加热的 C/SiC 板上的瞬态温度场。这两个代表性场景的验证表明了实时推理以及相对于传统插值器、传统神经网络和其他算子主干的持续改进。主要结论如下：
> 
> • 在钝锥体热通量情况下，融合离散测量、飞行状态变量和低保真CFD 产生低于2% 的平均相对误差，并且消融研究证实了每个组件的贡献。 • 在C/SiC 板温度情况下，以25 秒为间隔使用最近的三个测量值可将瞬态温度重建的平均相对误差降低至1.5% 以下，而忽略历史记录则将其提高至约6.4%。 • 训练后的模型在约0.08 秒内推理出一个字段，从而实现满足实时要求的在线更新。
> 
> 与最近邻插值、RBF 插值、BPNN、BPNN + POD 和替代骨干网络相比，TFNO 在两项任务上都实现了准确性和空间一致性之间的最佳平衡。使用固定掩蔽比的重复测试表明，即使某些传感器发生故障或位于未感测的位置，该模型仍然准确。
...
- 141. Cleary, J.W.,
- 1969. Effects of angle of attack and bluntness on laminar heating-rate distributions of a 15 cone at a mach number of 10.
- 6. Natl. Aeronaut. Space Administ. Faroughi S A, Pawar N, Fernandes C, et al. Physics-guided, physics-informed, and physics-encoded neural networks in scientific computing. arXiv preprint arXiv: 2211.07377,
- 2022. Forrester, A.I.J., S´obester, A., Keane, A.J.,
- 2007. Multi-fidelity optimization via surrogate modelling. Proc. Royal Soc. A: Math. Phys. Eng. Sci. 463 (2088), 3251–

### 20.9 常用词、词类、语态与时态

- 高频词：heat(89)；measurements(79)；sensor(63)；field(62)；reconstruction(59)；fig(59)；temperature(57)；data(51)；accuracy(50)；error(48)；tfno(46)；low-fidelity(46)；thermal(43)；sensors(41)；relative(40)；flux(38)；cfd(38)；input(38)；table(35)；fields(34)
- 高频名词化/学术名词：reconstruction(59)；temperature(57)；low-fidelity(46)；measurement(20)；function(19)；distribution(18)；information(17)；replication(17)；fidelity(16)；prediction(14)；performance(14)；tion(14)；smoothness(12)；validation(12)；conduction(11)
- 高频学术动词：predicted(16)；predict(6)；optimized(4)；derived(4)；demonstrate(4)；compare(4)；develop(3)；propose(3)；indicate(3)；compared(3)；validated(2)；estimate(2)；presented(2)；reveal(2)；showed(1)
- 高频形容词：thermal(43)；relative(40)；table(35)；neural(34)；spatial(29)；journal(25)；international(24)；global(23)；measurement(20)；transient(19)；spectral(18)；physical(17)；experimental(16)；available(15)；boundary(15)
- 高频副词：only(13)；respectively(10)；randomly(7)；effectively(7)；spatially(6)；significantly(6)；approximately(5)；typically(4)；fully(4)；primarily(4)；highly(4)；tively(4)；substantially(4)；physically(4)；subsequently(4)
- 高频二词短语：heat flux(34)；relative error(26)；international journal(24)；journal heat(24)；heat fluid(24)；fluid flow(24)；page wenyu(21)；wenyu international(21)；discrete measurements(18)；reconstruction accuracy(17)；rmse relative(15)；flow fig(14)
- 高频三词短语：international journal heat(24)；journal heat fluid(24)；heat fluid flow(24)；page wenyu international(21)；wenyu international journal(21)；fluid flow fig(14)；rmse relative error(11)；fourier neural operator(8)；mean relative error(8)；tensorized fourier neural(7)；measurements flight states(7)；retained spectral modes(6)
- 被动语态估计：98；`we + 动作动词` 主动句估计：9
- 一般现在时线索：242；一般过去时线索：533；现在完成时线索：1；情态动词线索：29

章节词频：

- Abstract: reconstruction(4)；number(4)；temperature(3)；thermal(3)；sensors(3)；sensor(3)；continued(3)；tensor(3)
- Introduction: sensor(13)；heat(12)；measurements(11)；fields(10)；boundary(9)；data(9)；flux(9)；thermal(8)
- Conclusion: measurements(3)；temperature(3)；tfno(2)；fields(2)；sparse(2)；discrete(2)；sensors(2)；flight-state(2)

### 20.10 句型库扩充（每类多句）

#### 背景句
- 原句/结构：Real-time monitoring of the heat flux and temperature of hypersonic flight vehicles is critical for safety, control, and thermal management.
  可迁移模板：Real-time monitoring of the heat flux and temperature of Y is critical for safety, control, and thermal management.
- 原句/结构：The situation becomes more challenging in extreme environments such as forebody leading edges, where sensor instrumentation is impractical and critical thermal regions remain unmeasured.
...
#### Gap句
- 原句/结构：In flight, however, sensing is typically limited to sparsely distributed sensors, which yields incomplete spatial coverage and incomplete information.
...
- 原句/结构：The situation becomes more challenging in extreme environments such as forebody leading edges, where sensor instrumentation is impractical and critical thermal regions remain unmeasured.
  可迁移模板：The situation becomes more challenging in extreme environments such as forebody leading edges, where sensor instrumentation is impractical and critical thermal regions remain unmeasured.
- 原句/结构：Full-field measurement techniques such as infrared thermography and temperature-sensitive paint are difficult to deploy in flight because of accuracy limits and the requirement for ground-based systems.
...
#### 方法句
- 原句/结构：To address these issues, we develop an enhanced method based on the tensorized Fourier neural operator (TFNO) that reconstructs full thermal fields from sparse measurements.
...
- 原句/结构：First, to mitigate the paucity of measurements, we propose a replication-masking strategy that expands the training set by randomly occluding a fraction of sensor readings in repeated copies.
...
- 原句/结构：Second, if state information is available, we feed it into the model together with the corresponding low-fidelity data to improve the reconstruction.
...
#### 结果句
- 原句/结构：Yang et al. (Yang and Fu, 2010) and Zhao et al. (Zhao et al., 2014) applied Tikhonov regula rization to estimate boundary heat flux and demonstrated this stabili zation in practice.
...
- 原句/结构：They showed that near-boundary sensors carry the greatest value and that inference can be performed in milliseconds without repeatedly solving the for ward model, though strongly nonlinear or non-unique problems may still demand more data or richer architectures.
...
- 原句/结构：There is a need for an approach that incorporates physical information and still achieves high-accuracy, real-time recon struction when measurements are taken at arbitrary locations.
...
#### 贡献句
- 原句/结构：To handle high-dimensional spatiotemporal thermal fields efficiently, conjugate-gradient sequential estimation with an adjoint model provides efficient gradients and makes the optimization fast.
...
- 原句/结构：Once trained, it provides fast, accurate on line estimates, but the offline cost is high and generalization beyond the sampled envelope is limited.
...
- 原句/结构：Tamaddon et al. (Tamaddon-Jahromi et al., 2020) established a back-propagation neural network (BPNN) model from discrete temperatures to boundary temperatures for multiple conduction and convection problems and sensor layouts.
...
- 原句/结构：In flight, however, sensing is typically limited to sparsely distributed sensors, which yields incomplete spatial coverage and incomplete information.
  可迁移模板：In flight, however, sensing is typically limited to sparsely distributed sensors, which yields incomplete spatial coverage and incomplete information.
- 原句/结构：Reconstructing a global thermal field (i.e., distri bution) from such limited sparse measurements constitutes an inverse problem.
...
- 原句/结构：To ensure solution stability, these methods are commonly combined with regularization.

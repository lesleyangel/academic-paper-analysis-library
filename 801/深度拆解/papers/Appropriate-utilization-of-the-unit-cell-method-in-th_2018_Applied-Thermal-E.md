...
> 自动分析说明：以下基于 `801/文本/txt/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.txt` 的章节标题抽取与标题关键词判断生成；PDF 抽取可能漏掉跨行小标题，最终章节名仍建议结合原 PDF 目录和版面核查。
...
> 本区块由 `tools/upgrade_801_deep_analysis.py` 基于 `801/文本/txt/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.txt` 与 `801/文本/metadata/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.json` 重新抽取生成；用于修正旧报告中章节未全、引文缺失、摘要/结论未完整摘录的问题。双栏 PDF 抽取仍可能存在断行，引用和公式编号以 PDF 版面为最终准绳。

### 20.1 严格章节树（按 PDF/metadata TOC）

- L2 p.1: Introduction （背景/领域定位）
- L2 p.2: Constitutive equations （对象/问题/模块）
- L2 p.2: Two formulation paths of the unit cell （方法/模型）
  - L3 p.2: UD composite （对象/问题/模块）
  - L3 p.3: Plain woven composite （对象/问题/模块）
  - L3 p.3: Three-dimensional four-directional braided composite （对象/问题/模块）
  - L3 p.4: Path summary and unit cell models （方法/模型）
- L2 p.4: Boundary conditions from different paths （对象/问题/模块）
  - L3 p.4: Macro thermal stimuli （对象/问题/模块）
  - L3 p.6: Boundary conditions from Path a （对象/问题/模块）
  - L3 p.6: Boundary conditions from Path b （对象/问题/模块）
- L2 p.8: Model discretization （方法/模型）
- L2 p.8: Results and discussions （结果/讨论/验证）
  - L3 p.8: UD composites （对象/问题/模块）
  - L3 p.8: Plain woven composites （对象/问题/模块）
  - L3 p.9: 3D four-directional braided composites （对象/问题/模块）
  - L3 p.10: Summary （对象/问题/模块）
- L2 p.10: Conclusions （结论）
...
| Constitutive equations | 2 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Two formulation paths of the unit cell | 2 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| UD composite | 2 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Plain woven composite | 3 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Three-dimensional four-directional braided composite | 3 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Path summary and unit cell models | 4 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Boundary conditions from different paths | 4 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Macro thermal stimuli | 4 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Boundary conditions from Path a | 6 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Boundary conditions from Path b | 6 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Model discretization | 8 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Results and discussions | 8 | 2 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| UD composites | 8 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Plain woven composites | 8 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3D four-directional braided composites | 9 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Summary | 10 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Conclusions | 10 | 2 | 结论 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
> 公开库不直接展示完整英文摘要原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。
...
> 公开库不直接展示完整英文结论原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.json` 的 `conclusion` 字段，以及 `801/深度拆解/local_full_reports/Appropriate-utilization-of-the-unit-cell-method-in-th_2018_Applied-Thermal-E.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。

### 20.6 结论中文翻译

> 本文对晶胞法在复合材料热计算中的应用进行了研究，阐明了不同边界条件下晶胞构型的混淆和均匀温度边界条件的应用范围两个问题。研究了单向纤维增强复合材料、平纹编织复合材料和三维四向编织复合材料三种类型的复合材料。对于每种复合材料，都会制定一个晶胞，并根据两种不同的制定路径导出两种类型的边界条件：平移对称性的路径 a 以及平移与反射或旋转对称性组合的路径 b。数值结果显示了一些结论：
> 
> 1. 晶胞可以通过不同的路径表示，并且可以具有不同的边界条件，而数值结果是唯一的。对于这样的晶胞，只要严格推导边界条件，就不需要关心具体的公式路径。因此，人们可以选择一条表述路径并导出适合自己工作的相应边界条件。 2.
> 
> 对于可以用反射对称性表示的晶胞和边界，广泛使用的均匀温度边界条件（UBC）是正确的，而对于只能用平移对称性或180°旋转对称性表示的晶胞来说，它是不合适的。对于 UD 复合材料，UBC 对于各个方向的热计算都是正确的；对于平纹编织复合材料，x、y 方向正确，z 方向不合适；对于三维四向编织复合材料，不适合全方向热计算。对于某些问题，UBC 可以是一个良好且简单的近似值，因为温度不均匀性可能相对较小，但是，必须首先评估可能的误差。 3. 计算出的边界面上的温度场总是表明进一步的结构对称性，可用于构建更小尺寸的晶胞。 J.-J。郭等人。应用热工139（2018）295–306
> 
> J.-J。郭等人。应用热工139（2018）295–306
...
- 旧方法/已有研究不足：For some problems, the UBC can be a good and easy approximation since the temperature nonuniformity could be relatively small, however, it is essential to evaluate possible errors first. 3.
...
- 学术/工程增量：For such a unit cell, the specific formulation path is not concerned provided that the boundary condition is derived rigorously.
...
- 引用簇样例：[1,2], [31], [3,4], [12], [32], [13,14], [20], [32,33], [34], [24,35,36], [32], [20]
...
- [11] H. Liu, X. Xia, X. Xie, Q. Ai, D. Li, Experiment and identification of thermal conductivity and extinction coefficient of silica aerogel composite, Int. J. Therm. Sci. 121 (Supplement C) (2017) 192–203.
...
- 高频词：boundary(72)；symmetries(68)；composites(64)；composite(50)；sts(49)；unit(47)；path(44)；fig(44)；planes(42)；temperature(41)；woven(41)；thermal(40)；plain(38)；braided(36)；transl(32)；formulation(31)；paths(29)；structure(29)；ats(29)；adiabatic(29)
- 高频名词化/学术名词：temperature(41)；formulation(31)；structure(29)；direction(20)；condition(12)；derivation(12)；ection(11)；calculation(9)；rotation(9)；z-direction(9)；guration(7)；y-direction(6)；periodicity(6)；utilization(5)；uence(5)
- 高频学术动词：derived(25)；indicated(9)；derive(8)；compared(3)；compare(2)；indicate(1)
- 高频形容词：boundary(72)；thermal(40)；adiabatic(29)；translational(28)；erent(26)；periodic(26)；ectional(25)；symmetric(21)；table(20)；rotational(17)；four-directional(12)；axial(9)；derive(8)；numerical(8)；typical(7)
- 高频副词：respectively(20)；only(7)；separately(5)；closely(4)；mainly(4)；randomly(3)；relatively(3)；apply(3)；totally(3)；rigorously(3)；widely(2)；tively(2)；directly(2)；nally(2)；imply(2)
- 高频二词短语：plain woven(36)；unit cell(27)；boundary planes(25)；boundary conditions(24)；braided composites(21)；sts sts(21)；transl transl(20)；unit cells(19)；sts ats(16)；translational symmetries(15)；structure symmetries(14)；ats sts(14)
- 高频三词短语：applied thermal engineering(13)；page gou applied(11)；gou applied thermal(11)；plain woven composite(11)；ats sts sts(11)；plain woven braided(10)；transl transl transl(10)；sts sts ats(10)；sts ats sts(10)；four-directional braided composites(8)；plain woven composites(8)；sts sts sts(8)
- 被动语态估计：121；`we + 动作动词` 主动句估计：0
- 一般现在时线索：198；一般过去时线索：303；现在完成时线索：0；情态动词线索：97

章节词频：

- Abstract: unit(5)；boundary(5)；cell(4)；conditions(4)；formulation(3)；composites(3)；calculations(2)；composite(2)
- Introduction: composites(17)；thermal(11)；symmetries(11)；unit(10)；boundary(9)；temperature(8)；rve(8)；structure(8)
- Conclusion: unit(8)；boundary(8)；composites(6)；cell(5)；thermal(5)；symmetries(5)；conditions(4)；temperature(4)

### 20.10 句型库扩充（每类多句）

#### 背景句
- 原句/结构：For some problems, the UBC can be a good and easy approximation since the temperature nonuniformity could be relatively small, however, it is essential to evaluate possible errors first. 3.
...
#### Gap句
- 原句/结构：Compare with the RVE of randomly structured composites, a UC model for symmetrically structured composites can be theoretically accurate in representing the macro composite; however, the formulation of an accurate UC is relatively complicated.
...
- 原句/结构：First, in previous literatures, the utilization of different combination of symmetries (expressed as UC formulation path in this work) means different configurations and different BC of the UC; however, sometimes different symmetries may formulate the same geometric configuration and meanwhile the derived BC are still different; this brings about some confusions in simulations and needs to be clarified.
...
#### 方法句
- 原句/结构：The unit cell method is widely used in thermal calculations of composite.
  可迁移模板：The unit cell method is widely used in thermal calculations of composite.
- 原句/结构：In this work, two issues are studied to further appropriate utilization of such method.
  可迁移模板：In this work, two issues are studied to further appropriate utilization of such method.
- 原句/结构：The effective thermal properties of composites can be efficiently calculated by a representative volume element (RVE) model.
...
- 原句/结构：The numerical results show some conclusions: 1.
  可迁移模板：The numerical results show some conclusions: X.
- 原句/结构：A unit cell can be formulated by different paths and can have different boundary conditions, whereas the numerical results are unique.
...
#### 贡献句
- 原句/结构：For such a unit cell, the specific formulation path is not concerned provided that the boundary condition is derived rigorously.
...
- 原句/结构：Under this condition, a reliable and efficient thermal protection system (TPS) is required, and the thermal characteristics of relevant TPS composites should be deeply studied.

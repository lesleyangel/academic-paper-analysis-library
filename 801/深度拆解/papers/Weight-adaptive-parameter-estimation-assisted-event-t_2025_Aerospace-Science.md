...
> 自动分析说明：以下基于 `801/文本/txt/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.txt` 的章节标题抽取与标题关键词判断生成；PDF 抽取可能漏掉跨行小标题，最终章节名仍建议结合原 PDF 目录和版面核查。
...
| --- | --- | --- | --- | --- | --- |
| 1 Introduction                                                    With the rapid development of embedded computing capabilities, | 背景/引言型 | 建立问题背景、研究动机和文献缺口 | 高 | 是 | 保留具体变量/对象 |
| 2.3 Reentry guidance problem | 描述型 | 描述章节内容，信息量取决于标题具体程度 | 中 | 是 | 可加入核心变量或机制词增强信息量 |
| 6.2 Monte Carlo simulation | 方法/模型型 | 交代模型、公式、算法、参数或求解流程 | 中 | 是 | 可加入核心变量或机制词增强信息量 |
| 7 Conclusions                                                Data availability | 结论/展望型 | 收束贡献、边界和未来工作 | 中 | 是 | 可加入核心变量或机制词增强信息量 |
...
- Top 高频词：guidance(128)；model(57)；parameter(54)；state(54)；et-nmpg(50)；control(48)；estimation(46)；flight(46)；reentry(41)；data(41)；org(38)；zhang(36)；nmpg(35)；trajectory(33)；parameters(33)；cos(25)；method(24)；results(23)；mesh(23)；terminal(22)
- 高频学术名词：guidance(128)；model(57)；parameter(54)；estimation(46)；simulation(36)；parameters(33)；method(24)；results(23)；deviation(21)；optimization(19)；section(17)；performance(16)；science(14)；system(13)；solution(12)；reference(12)
- 高频学术动词：shown(8)；proposed(8)；estimated(5)；shows(5)；estimate(4)；compared(3)；demonstrate(3)；simulate(3)；propose(3)；compare(3)；show(2)；solve(2)；predicted(1)；indicate(1)
- 高频形容词：dynamic(32)；terminal(22)；aerodynamic(20)；initial(19)；predictive(17)；cient(17)；event(14)；erent(14)；optimal(12)；atmospheric(12)；computational(9)；subsequent(9)；hypersonic(9)；substantial(8)；actual(8)；relative(8)
- 高频副词/连接副词：however(10)；consequently(6)；respectively(6)；relatively(6)；cally(5)；finally(5)；cantly(5)；therefore(4)；generally(4)；usually(4)；slightly(4)；accurately(4)；early(4)；highly(3)；primarily(3)；fully(3)
- 高频二词短语：parameter estimation(28)；guidance law(16)；wape-assisted et-nmpg(16)；reentry guidance(15)；zhang gong(15)；aerospace science(14)；science technology(14)；model predictive(14)；trajectory optimization(14)；flight state(13)；zhang zhang(12)；gong aerospace(12)；guidance commands(11)；initial state(11)；terminal state(10)；coe cient(10)
- 高频三词短语：aerospace science technology(14)；zhang zhang gong(12)；zhang gong aerospace(12)；gong aerospace science(12)；guid control dyn(9)；aerosp sci technol(8)；model predictive control(7)；et-nmpg wape-assisted et-nmpg(7)；control dyn org(6)；event-triggered model predictive(5)；change point detection(5)；open-loop guidance law(5)
...
- Abstract/首页：guidance(10)；parameter(5)；model(5)；aerospace(3)；estimation(3)；event-triggered(3)；predictive(3)；reentry(3)
- 1. Introduction                                                    With the rapid development of embedded computing capabilities,：guidance(74)；state(45)；model(35)；flight(33)；data(30)；parameter(29)；reentry(28)；nmpg(28)
- 6.2. Monte Carlo simulation：guidance(23)；estimation(19)；et-nmpg(18)；parameter(16)；terminal(11)；errors(10)；results(10)；wape-assisted(9)
- 7. Conclusions                                                Data availability：org(37)；control(30)；guidance(21)；model(12)；guid(12)；aerosp(11)；sci(11)；dyn(11)
...
> 本区块由 `tools/upgrade_801_deep_analysis.py` 基于 `801/文本/txt/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.txt` 与 `801/文本/metadata/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.json` 重新抽取生成；用于修正旧报告中章节未全、引文缺失、摘要/结论未完整摘录的问题。双栏 PDF 抽取仍可能存在断行，引用和公式编号以 PDF 版面为最终准绳。

### 20.1 严格章节树（按 PDF/metadata TOC）

- L2 p.1: 1 Introduction （背景/领域定位）
- L2 p.2: 2 Problem formulation （方法/模型）
  - L3 p.2: 2.1 Standard motion equations （对象/问题/模块）
  - L3 p.3: 2.2 Deviations and disturbances （对象/问题/模块）
  - L3 p.3: 2.3 Reentry guidance problem （对象/问题/模块）
- L2 p.3: 3 The ET-NMPG method （方法/模型）
  - L3 p.3: 3.1 Definition of TOP （对象/问题/模块）
  - L3 p.4: 3.2 Process of ET-NMPG （对象/问题/模块）
  - L3 p.4: 3.3 Tricks for TO online （对象/问题/模块）
    - L4 p.4: 3.3.1 Mesh refinement or not （对象/问题/模块）
    - L4 p.4: 3.3.2 Mesh scheme and initial value （对象/问题/模块）
    - L4 p.4: 3.3.3 State prediction before NMPG （对象/问题/模块）
- L2 p.5: 4 WAPE and change point detection （对象/问题/模块）
  - L3 p.5: 4.1 PE for atmosphere （对象/问题/模块）
  - L3 p.5: 4.2 PE for aerodynamics （对象/问题/模块）
  - L3 p.6: 4.3 Change point detection （对象/问题/模块）
- L2 p.7: 5 The guidance framework （方法/模型）
- L2 p.7: 6 Simulation results and discussion （结果/讨论/验证）
  - L3 p.7: 6.1 Comparison of different methods （方法/模型）
  - L3 p.9: 6.2 Monte Carlo simulation （对象/问题/模块）
  - L3 p.11: 6.3 Results with in-flight faults （结果/讨论/验证）
- L2 p.12: 7 Conclusions （结论）
...
| 2.1 Standard motion equations | 2 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.2 Deviations and disturbances | 3 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 2.3 Reentry guidance problem | 3 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3 The ET-NMPG method | 3 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.1 Definition of TOP | 3 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.2 Process of ET-NMPG | 4 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.3 Tricks for TO online | 4 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.3.1 Mesh refinement or not | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.3.2 Mesh scheme and initial value | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 3.3.3 State prediction before NMPG | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4 WAPE and change point detection | 5 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4.1 PE for atmosphere | 5 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4.2 PE for aerodynamics | 5 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 4.3 Change point detection | 6 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 5 The guidance framework | 7 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 6 Simulation results and discussion | 7 | 2 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 6.1 Comparison of different methods | 7 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 6.2 Monte Carlo simulation | 9 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 6.3 Results with in-flight faults | 11 | 3 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 7 Conclusions | 12 | 2 | 结论 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
> 公开库不直接展示完整英文摘要原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。
...
> 公开库不直接展示完整英文结论原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.json` 的 `conclusion` 字段，以及 `801/深度拆解/local_full_reports/Weight-adaptive-parameter-estimation-assisted-event-t_2025_Aerospace-Science.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。
...
- 旧方法/已有研究不足：However, the actual model parameter values often deviate from the reference values, leading to an excessively high event trigger frequency and potentially rendering the trajectory optimization unfeasible. Importantly, for reentry processes, this approach can further ensure that the flight state remains within acceptable path constraints. However, people believe that as technology contin ues to advance, winged vehicles will eventually regain their relevance in space transportation [2].
...
- 引用簇样例：[7], [8,9], [1], [14], [2], [3], [4], [20], [5,6], [24], [25,26], [27]
...
- [8] R. Chai, K. Chen, L. Cui, S. Chai, G. Inalhan, A. Tsourdos, Advanced Trajectory Op
timization, Guidance and Control Strategies for Aerospace Vehicles: Methods and Applications, Springer Aerospace Technology, Springer Nature, Singapore, 2023.
- [9] H. Xu, G. Cai, C. Mu, X. Li, Analytical reentry guidance framework based on swarm intelligence optimization and altitude-energy prfile, Chin. J. Aeronaut. 36 (12) (2023) 336--348, https://doi.org/10.1016/j.cja.2023.07.029.
- [10] C. Sánchez, D. Izzo, Real-time optimal control via deep neural networks: study on landing problems, J. Guid. Control Dyn. 41 (5) (2018) 1122--1135, https://doi.org/ 10.2514/1.G002357.
- [11] R. Chai, A. Tsourdos, A. Savvaris, Y. Xia, S. Chai, Real-time reentry trajectory plan
ning of hypersonic vehicles: a two-step strategy incorporating fuzzy multiobjective transcription and deep neural network, IEEE Trans. Ind. Electron. 67 (8) (2020) 6904--6915, https://doi.org/10.1109/TIE.2019.2939934.

### 20.9 常用词、词类、语态与时态

- 高频词：guidance(109)；state(54)；parameter(50)；et-nmpg(48)；flight(43)；estimation(42)；data(35)；nmpg(33)；reentry(32)；parameters(32)；zhang(27)；trajectory(27)；fig(26)；cos(25)；when(23)；where(23)；mesh(23)；not(21)；deviations(21)；deviation(20)
- 高频名词化/学术名词：guidance(109)；estimation(42)；deviation(20)；simulation(17)；section(17)；optimization(15)；performance(15)；science(13)；reference(12)；motion(11)；detection(10)；density(9)；function(8)；mutation(8)；position(7)
- 高频学术动词：estimate(4)；estimated(4)；propose(3)；compare(3)；demonstrate(2)；presented(2)；compared(2)；optimized(2)；predicted(1)；indicate(1)
- 高频形容词：terminal(20)；initial(19)；aerodynamic(17)；cient(16)；table(15)；dynamic(14)；erent(14)；event(13)；atmospheric(11)；subsequent(9)；predictive(8)；optimal(7)；interval(7)；constant(7)；real(7)
- 高频副词：only(11)；respectively(6)；relatively(6)；cally(5)；finally(5)；cantly(5)；usually(4)；slightly(4)；accurately(4)；early(4)；highly(3)；primarily(3)；fully(3)；mainly(3)；strictly(3)
- 高频二词短语：parameter estimation(24)；guidance law(16)；wape-assisted et-nmpg(16)；aerospace science(13)；science technology(13)；flight state(13)；reentry guidance(12)；trajectory optimization(11)；guidance commands(11)；page zhang(11)；zhang zhang(11)；zhang gong(11)
- 高频三词短语：aerospace science technology(13)；page zhang zhang(11)；zhang zhang gong(11)；zhang gong aerospace(11)；gong aerospace science(11)；et-nmpg wape-assisted et-nmpg(7)；change point detection(5)；open-loop guidance law(5)；science technology fig(5)；aerodynamic coe cients(5)；reentry guidance problem(4)；initial state deviation(4)
- 被动语态估计：84；`we + 动作动词` 主动句估计：5
- 一般现在时线索：184；一般过去时线索：284；现在完成时线索：3；情态动词线索：50

章节词频：

- Abstract: guidance(8)；parameter(3)；accuracy(3)；wape(2)；reentry(2)；online(2)；trajectory(2)；optimization(2)
- Introduction: guidance(39)；reentry(14)；control(13)；flight(11)；online(11)；parameter(10)；nmpg(10)；trajectory(8)
- Conclusion: guidance(5)；et-nmpg(4)；nmpg(3)；wape-assisted(2)；online(2)；event-triggered(2)；accuracy(2)；parameter(2)

### 20.10 句型库扩充（每类多句）

#### 背景句
- 原句/结构：Importantly, for reentry processes, this approach can further ensure that the flight state remains within acceptable path constraints.
...
#### Gap句
- 原句/结构：However, the actual model parameter values often deviate from the reference values, leading to an excessively high event trigger frequency and potentially rendering the trajectory optimization unfeasible.
...
- 原句/结构：Importantly, for reentry processes, this approach can further ensure that the flight state remains within acceptable path constraints.
  可迁移模板：Importantly, for reentry processes, this approach can further ensure that the flight state remains within acceptable path constraints.
- 原句/结构：However, people believe that as technology contin ues to advance, winged vehicles will eventually regain their relevance in space transportation [2].
...
#### 方法句
- 原句/结构：Communicated by Xingqun Zhan This paper proposes an event-triggered model predictive guidance (ET-NM PG) method assisted by weight adaptive parameter estimation (WAPE) and applies it to reentry guidance.
...
- 原句/结构：Guidance methods based on online trajectory optimization (TO) often face a trade-off between guidance accuracy and the efficiency of guidance command computation when dealing with complex problems.
...
- 原句/结构：However, the actual model parameter values often deviate from the reference values, leading to an excessively high event trigger frequency and potentially rendering the trajectory optimization unfeasible.
  可迁移模板：However, the actual model parameter values often deviate from the reference values, leading to an excessively high event trigger frequency and potentially rendering the trajectory optimization unfeasible.
#### 结果句
- 原句/结构：The numerical simulation results cofirm the effectiveness of the proposed method.
  可迁移模板：The numerical simulation results cofirm the effectiveness of the proposed method.
- 原句/结构：This results in a wide range of state dispersion during and at the end of the reentry flight, which is not conducive to safe landing and reuse of the vehicle. * Corresponding author.
...
- 原句/结构：The numerical simulation results cofirm the effectiveness of the proposed method.
  可迁移模板：The numerical simulation results cofirm the effectiveness of the proposed method.
#### 贡献句
- 原句/结构：We cannot expect the NMPG to provide guidance commands in real-time like traditional algorithms, and current research has primarily employed two strate gies for generating guidance commands through TO: the uninterrupted NMPG (UI-NMPG) strategy [24], which immediately carries out TO af ter completing the previous one; and the fixed cycle NMPG (FC-NPMGP) strategy [25,26], which establishes a longer interval to ensure optimal guidance law generation within that period.
  可迁移模板：We cannot expect the METHOD to provide guidance commands in real-time like traditional algorithms, and current research has primarily employed two strate gies for generating guidance commands through METHOD: the uninterrupted METHOD (METHOD) strategy [X], which immediately carries out METHOD af ter completing the previous one; and the fixed cycle METHOD (METHOD) strategy [X,X], which establishes a longer interval to ensure optimal guidance law generation within that period.
- 原句/结构：The standard reentry motion model, the motion model considering deviations and disturbances, and the reentry guidance problem are provided in Section 2.
...
#### 限制/边界句
- 原句/结构：In addition, since the reentry dynamics system is an underactuated system, only one to three state variables can be selected for tracking (e.g., drag acceleration, altitude, latitude, and lon gitude) when performing tracking guidance [5,6].

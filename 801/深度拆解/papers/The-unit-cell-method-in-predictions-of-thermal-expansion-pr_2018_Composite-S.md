...
> 自动分析说明：以下基于 `801/文本/txt/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.txt` 的章节标题抽取与标题关键词判断生成；PDF 抽取可能漏掉跨行小标题，最终章节名仍建议结合原 PDF 目录和版面核查。
...
> 本区块由 `tools/upgrade_801_deep_analysis.py` 基于 `801/文本/txt/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.txt` 与 `801/文本/metadata/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.json` 重新抽取生成；用于修正旧报告中章节未全、引文缺失、摘要/结论未完整摘录的问题。双栏 PDF 抽取仍可能存在断行，引用和公式编号以 PDF 版面为最终准绳。

### 20.1 严格章节树（按 PDF/metadata TOC）

- L2 p.1: Introduction （背景/领域定位）
- L2 p.2: Mathematical formulation （方法/模型）
  - L3 p.2: Constitutive equations （对象/问题/模块）
  - L3 p.3: Temperature and displacement distributions in symmetric structures （对象/问题/模块）
    - L4 p.3: Translational symmetric structure （对象/问题/模块）
    - L4 p.4: Reflectional symmetric structure （对象/问题/模块）
    - L4 p.4: 180° rotational symmetric structure （对象/问题/模块）
    - L4 p.4: Summary of displacement distribution rules （对象/问题/模块）
    - L4 p.5: Application scopes （对象/问题/模块）
- L2 p.5: Development of unit cells （对象/问题/模块）
  - L3 p.6: Unit cells of fiber yarns （对象/问题/模块）
    - L4 p.6: Formulation of unit cells （方法/模型）
    - L4 p.7: Boundary conditions （对象/问题/模块）
  - L3 p.7: Unit cells of plane woven composites （对象/问题/模块）
    - L4 p.7: Formulation of unit cells （方法/模型）
    - L4 p.8: Boundary conditions （对象/问题/模块）
  - L3 p.8: Unit cells of satin woven composites （对象/问题/模块）
    - L4 p.8: Formulation of unit cells （方法/模型）
    - L4 p.9: Boundary conditions （对象/问题/模块）
  - L3 p.9: Summary （对象/问题/模块）
- L2 p.9: Numerical modeling （方法/模型）
  - L3 p.9: Application of boundary conditions （对象/问题/模块）
  - L3 p.11: Model discretization （方法/模型）
  - L3 p.13: Material properties （对象/问题/模块）
- L2 p.13: Results and discussion （结果/讨论/验证）
  - L3 p.13: Thermal expansion of UD composite （对象/问题/模块）
    - L4 p.13: Thermal expansion behaviors （对象/问题/模块）
    - L4 p.13: Effective CTE and validation of numerical models （方法/模型）
  - L3 p.13: Thermal expansion of plain woven composite （对象/问题/模块）
    - L4 p.13: Thermal expansion behaviors （对象/问题/模块）
    - L4 p.14: Effective CTE and validation of boundary conditions （结果/讨论/验证）
  - L3 p.14: Thermal expansion of satin woven composite （对象/问题/模块）
    - L4 p.14: Thermal expansion behaviors （对象/问题/模块）
    - L4 p.15: Effective CTE and validation of boundary conditions （结果/讨论/验证）
  - L3 p.15: Normal displacement on boundary planes （对象/问题/模块）
- L2 p.17: Conclusions （结论）
- L2 p.18: Acknowledgment （对象/问题/模块）
...
| Mathematical formulation | 2 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Constitutive equations | 2 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Temperature and displacement distributions in symmetric structures | 3 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Translational symmetric structure | 3 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Reflectional symmetric structure | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| 180° rotational symmetric structure | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Summary of displacement distribution rules | 4 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Application scopes | 5 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Development of unit cells | 5 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Unit cells of fiber yarns | 6 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Formulation of unit cells | 6 | 4 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Boundary conditions | 7 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Unit cells of plane woven composites | 7 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Formulation of unit cells | 7 | 4 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Boundary conditions | 8 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Unit cells of satin woven composites | 8 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Formulation of unit cells | 8 | 4 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Boundary conditions | 9 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Summary | 9 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Numerical modeling | 9 | 2 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Application of boundary conditions | 9 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Model discretization | 11 | 3 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Material properties | 13 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Results and discussion | 13 | 2 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Thermal expansion of UD composite | 13 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Thermal expansion behaviors | 13 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Effective CTE and validation of numerical models | 13 | 4 | 方法/模型 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Thermal expansion of plain woven composite | 13 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Thermal expansion behaviors | 13 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Effective CTE and validation of boundary conditions | 14 | 4 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Thermal expansion of satin woven composite | 14 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Thermal expansion behaviors | 14 | 4 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Effective CTE and validation of boundary conditions | 15 | 4 | 结果/讨论/验证 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Normal displacement on boundary planes | 15 | 3 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Conclusions | 17 | 2 | 结论 | 来自 metadata TOC，正式分析按此章节点名复核 |
| Acknowledgment | 18 | 2 | 对象/问题/模块 | 来自 metadata TOC，正式分析按此章节点名复核 |
...
> 公开库不直接展示完整英文摘要原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。
...
> 公开库不直接展示完整英文结论原文；完整摘录保存在本地忽略目录 `801/深度拆解/extracted_evidence/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.json` 的 `conclusion` 字段，以及 `801/深度拆解/local_full_reports/The-unit-cell-method-in-predictions-of-thermal-expansion-pr_2018_Composite-S.md`。本节保留抽取状态、中文译文和分析内容，便于公开阅读与本地复核。

### 20.6 结论中文翻译

> 这项工作开发了一种制定晶胞的方法来预测
> 
> J.-J。郭等人。复合结构195（2018）99–117
> 
> UD UC1 统一。统一。统一。统一。统一。统一。 UC2 统一。统一。统一。统一。统一。统一。 UC3 统一。统一。统一。统一。统一。统一。 UC4 统一。统一。统一。不统一。统一。统一。平纹编织 UC1 Unif。统一。统一。统一。不统一。不统一。 UC2 统一。统一。统一。统一。不统一。不统一。 UC3 统一。不统一。统一。不统一。不统一。不统一。缎纹编织 UC1 非统一。不统一。不统一。不统一。不统一。不统一。 UC2 非统一。不统一。不统一。不统一。不统一。不统一。制定了缎纹编织复合材料的较小晶胞，并阐明了晶胞边界上的变形模式及其与结构对称性的关系。可以总结出以下几个结论：
> 
> 1. 对于具有对称结构的复合材料，可以通过晶胞模型有效计算有效热膨胀系数。这种晶胞应该基于结构对称性来制定，并且可以根据本工作中总结的位移分布规则导出相关的BC。这种晶胞模型还可用于预测纤维纱线或纺织增强复合材料中的局部应力分布，因此具有对这些复合材料进行损伤分析的潜力。 2.
> 
> 晶胞的边界条件是基于均匀宏观应变场的假设推导的，该宏观应变场可以由均匀的温度载荷产生。因此，目前的晶胞方法只能用于模拟均匀温度载荷下的热膨胀行为，例如特定热处理后的热残余应力。 3.对于可以用反射对称性表述的边界，可以在热膨胀计算中得到均匀法向位移和均匀变形；对于只能基于平移或旋转对称性制定的边界，法向位移和相关变形是不均匀的。

### 20.7 论文逻辑脉络复核

- 提出的问题：需结合 Introduction 首段复核。
- 旧方法/已有研究不足：Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell. Composite Structures 195 (2018) 99–117 Contents lists available at ScienceDirect Composite Structures journal homepage: www.elsevier.com/locate/compstruct T Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell.
...
- References 条目数（解析）：44
- 可识别年份条目数：43
- 近五年/近年文献（2021+）数量：0
- 经典文献（2010年前）数量：17
- 同刊引用数量（按 subject 粗略匹配）：1
- 高频来源期刊（粗略）：Composite Structures(1)
- 引用簇样例：[2], [3], [1], [3], [3], [4], [23], [24,25], [26], [27], [28], [22]
...
- [7] Dong K, Peng X, Zhang J, Gu B, Sun B. Temperature-dependent thermal expansion behaviors of carbon fiber/epoxy plain woven composites: Experimental and numerical studies. Compos Struct 2017;176:329–41.
...
- 高频词：thermal(14)；unit(12)；composites(11)；composite(10)；cell(10)；expansion(9)；structures(8)；rve(7)；properties(6)；structure(6)；should(5)；symmetries(4)；conditions(4)；ective(4)；mori-tanaka(4)；more(4)；pdf(3)；available(3)；reinforced(3)；aerospace(3)
- 高频名词化/学术名词：expansion(9)；structure(6)；university(3)；temperature(3)；application(2)；prediction(2)；element(2)；condition(2)；conduction(2)；extraction(1)；astronautics(1)；science(1)；education(1)；establishment(1)；derivation(1)
- 高频学术动词：developed(2)；derived(2)；predicted(1)；validated(1)；compared(1)
- 高频形容词：thermal(14)；ective(4)；available(3)；boundary(3)；symmetric(3)；numerical(3)；erent(3)；analytical(3)；cient(3)；physical(3)；relevant(2)；element(2)；journal(1)；polytechnical(1)；general(1)
- 高频副词：only(2)；mainly(2)；ciently(1)；respectively(1)；widely(1)；frequently(1)；carefully(1)；primarily(1)
- 高频二词短语：unit cell(9)；thermal expansion(8)；composite structures(4)；structure symmetries(3)；boundary conditions(3)；expansion properties(2)；properties textile(2)；textile reinforced(2)；reinforced composites(2)；key laboratory(2)；university china(2)；approach developed(2)
- 高频三词短语：thermal expansion properties(2)；expansion properties textile(2)；properties textile reinforced(2)；textile reinforced composites(2)；unit cell clari(2)；cell clari numerical(2)；thermal expansion coe(2)；source pdf the-unit-cell-method-in-predictions-of-thermal-expansion-pr(1)；pdf the-unit-cell-method-in-predictions-of-thermal-expansion-pr composite-s(1)；the-unit-cell-method-in-predictions-of-thermal-expansion-pr composite-s pdf(1)；composite-s pdf extracted(1)；pdf extracted extraction(1)
- 被动语态估计：16；`we + 动作动词` 主动句估计：0
- 一般现在时线索：22；一般过去时线索：46；现在完成时线索：0；情态动词线索：15

章节词频：

- Abstract: unit(7)；thermal(5)；cell(5)；expansion(4)；composites(4)；structure(3)；symmetries(3)；three(3)
- Introduction: unit(24)；thermal(20)；cell(20)；composites(17)；composite(17)；symmetries(12)；structures(11)；structure(10)
- Conclusion: unif(33)；non-unif(21)；unit(11)；thermal(7)；formulated(7)；cells(6)；expansion(5)；composites(5)

### 20.10 句型库扩充（每类多句）

#### 背景句
- 原句/结构：Therefore, a unit cell model has two essential elements, the identification of structure symmetries and the derivation of accurate BCs.
...
- 原句/结构：Boundary conditions are the relations between the micro unit cell model and the macro real composite (essential conditions of homogenization).
...
#### Gap句
- 原句/结构：Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell.
...
- 原句/结构：Composite Structures 195 (2018) 99–117 Contents lists available at ScienceDirect Composite Structures journal homepage: www.elsevier.com/locate/compstruct T Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell.
  可迁移模板：Composite Structures X(X) X–XContents lists available at ScienceDirect Composite Structures journal homepage: www.elsevier.com/locate/compstruct T Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell.
- 原句/结构：However, if the reflectional and rotational symmetries are involved, the BC would be more complex and its implementation would raise some particular requirement of mesh generations.
...
#### 方法句
- 原句/结构：In this paper, a general approach is developed for the establishment of such a unit cell model.
  可迁移模板：In this paper, a general approach is developed for the establishment of such a unit cell model.
- 原句/结构：The application scope of present unit cell method is clarified from the thermal and mechanical point of views.
  可迁移模板：The application scope of present unit cell method is clarified from the thermal and mechanical point of views.
- 原句/结构：The influence of structure symmetries on the deformation pattern of unit cell models is clarified.
  可迁移模板：The influence of structure symmetries on the deformation pattern of unit cell models is clarified.
#### 结果句
- 原句/结构：The numerical models are validated by the identical results obtained from unit cells of different sizes and also by the results available in literatures.
...
- 原句/结构：In this paper, a general approach is developed for the establishment of such a unit cell model.
  可迁移模板：In this paper, a general approach is developed for the establishment of such a unit cell model.
- 原句/结构：Li and his team also contribute a lot of extraordinary works on the unit cell method for mechanical studies of various textile reinforced composite [24,25].
...
- 原句/结构：Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell.
  可迁移模板：Thermal expansion properties of textile reinforced composites with certain structure symmetries can be efficiently calculated by a size-limited unit cell.
- 原句/结构：The temperature of certain location in highspeed aerospace vehicles may increase from room temperature to above 1000 °C in a short time of 600 s [1].

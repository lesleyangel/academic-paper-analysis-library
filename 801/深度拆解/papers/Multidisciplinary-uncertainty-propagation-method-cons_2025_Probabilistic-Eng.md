...
- 正文词频只统计正文主章节：1 Introduction, 2 Multidisciplinary uncertainty propagation method considering correlated field variables for the rocket system, 3 Method validation by the analytical test, 4 Multidisciplinary uncertainty propagation of a solid rocket system, 5 Conclusion。
...
- 摘要抽取状态：成功。
- 完整英文摘要原文不在公开报告正文中展开；本地完整摘录见 `801/深度拆解/extracted_evidence/Multidisciplinary-uncertainty-propagation-method-cons_2025_Probabilistic-Eng.json` 的 `abstract` 字段，以及 `801/深度拆解/local_full_reports/Multidisciplinary-uncertainty-propagation-method-cons_2025_Probabilistic-Eng.md`。
...
- L2 p.3: 2 Multidisciplinary uncertainty propagation method considering correlated field variables for the rocket system（方法/模型/算法）
  - L3 p.3: 2.1 Multidisciplinary uncertainty propagation process considering correlated field variables（对象/模块/过渡章节）
  - L3 p.4: 2.2 Correlation transformation method for both numerical and field samples（方法/模型/算法）
    - L4 p.5: 2.2.1 Improved Nataf transformation method based on warm-start-based maximum entropy（方法/模型/算法）
    - L4 p.5: 2.2.2 Nataf transformation（对象/模块/过渡章节）
    - L4 p.6: 2.2.3 Maximum entropy method（方法/模型/算法）
  - L3 p.6: 2.3 POD method（方法/模型/算法）
- L2 p.6: 3 Method validation by the analytical test（方法/模型/算法）
- L2 p.8: 4 Multidisciplinary uncertainty propagation of a solid rocket system（对象/模块/过渡章节）
  - L3 p.8: 4.1 Problem description（问题定义）
  - L3 p.9: 4.2 Uncertainty propagation analysis of the solid rocket system considering field variable correlations（对象/模块/过渡章节）
- L2 p.10: 5 Conclusion（结论/贡献回收）
- L2 p.11: CRediT authorship contribution statement（尾部材料）
...
| 2 Multidisciplinary uncertainty propagation method considering correlated field variables for the rocket system | 3 | 2 | 方法/模型/算法 |
| 2.1 Multidisciplinary uncertainty propagation process considering correlated field variables | 3 | 3 | 对象/模块/过渡章节 |
| 2.2 Correlation transformation method for both numerical and field samples | 4 | 3 | 方法/模型/算法 |
| 2.2.1 Improved Nataf transformation method based on warm-start-based maximum entropy | 5 | 4 | 方法/模型/算法 |
| 2.2.2 Nataf transformation | 5 | 4 | 对象/模块/过渡章节 |
| 2.2.3 Maximum entropy method | 6 | 4 | 方法/模型/算法 |
| 2.3 POD method | 6 | 3 | 方法/模型/算法 |
| 3 Method validation by the analytical test | 6 | 2 | 方法/模型/算法 |
| 4 Multidisciplinary uncertainty propagation of a solid rocket system | 8 | 2 | 对象/模块/过渡章节 |
| 4.1 Problem description | 8 | 3 | 问题定义 |
| 4.2 Uncertainty propagation analysis of the solid rocket system considering field variable correlations | 9 | 3 | 对象/模块/过渡章节 |
| 5 Conclusion | 10 | 2 | 结论/贡献回收 |
| CRediT authorship contribution statement | 11 | 2 | 尾部材料 |
...
- 正文统计字符数：29691
- 高频词：variables(77)；field(58)；samples(56)；method(47)；correlation(42)；numerical(33)；correlations(29)；transformation(27)；input(22)；coefficient(22)；uncertainty(21)；discipline(21)；correlated(21)；matrix(21)；rocket(20)；multidisciplinary(19)；variable(19)；standard(18)；nataf(17)；maximum(17)
- 高频名词化/学术名词：correlation(42)；transformation(27)；function(10)；reference(10)；propagation(8)；velocity(8)；distribution(8)；density(7)；probability(6)；deviation(6)；separation(6)；optimization(5)；section(5)；skewness(5)；performance(4)
- 高频学术动词：propose(3)；compared(3)；presented(3)；achieve(2)；indicate(2)；demonstrate(1)；provided(1)；achieved(1)；estimate(1)；estimated(1)；constructed(1)；validate(1)；evaluate(1)；enabled(1)
- 高频形容词：numerical(33)；coefficient(22)；multidisciplinary(19)；variable(19)；independent(15)；statistical(15)；relative(11)；normal(11)；current(9)；total(7)；previous(6)；aerodynamic(6)；different(4)；analytical(4)；moment(4)
- 高频副词：subsequently(5)；accurately(5)；significantly(4)；effectively(4)；usually(4)；explicitly(3)；directly(3)；typically(3)；similarly(3)；closely(3)；primarily(2)；notably(2)；systematically(1)；inherently(1)；properly(1)
- 高频二词短语：field variables(21)；field samples(18)；correlation coefficient(16)；maximum entropy(14)；nataf transformation(13)；input variables(11)；statistical moments(10)；coefficient matrix(10)；numerical samples(10)；standard normal(10)；relative error(10)；correlated field(9)
- 高频三词短语：maximum entropy method(8)；correlation coefficient matrix(7)；velocity total energy(6)；value relative error(6)；solid rocket system(5)；total energy first-stage(5)；energy first-stage separation(5)；correlated field variables(5)；numerical field variables(5)；correlated input samples(4)；probability density function(4)；first-stage separation point(4)
- 被动语态估计：65；`we + 动作动词` 主动句估计：7
- 一般现在时线索：129；一般过去时线索：214；现在完成时线索：2；情态动词线索：19

分章节正文词频：

- 1 Introduction: variables(23)；field(16)；correlations(14)；multidisciplinary(13)；method(12)；accuracy(8)；design(7)；systems(6)
- 2 Multidisciplinary uncertainty propagation method considering correlated field variables for the rocket system: samples(50)；field(26)；correlation(25)；matrix(21)；coefficient(20)；method(18)；transformation(18)；variables(17)
- 3 Method validation by the analytical test: variables(19)；subsystem(10)；subsystems(7)；correlation(7)；method(6)；input(6)；numerical(5)；state(5)
- 4 Multidisciplinary uncertainty propagation of a solid rocket system: variables(13)；rocket(10)；discipline(10)；uncertainty(10)；correlations(8)；length(7)；correlation(7)；relative(7)
- 5 Conclusion: field(7)；variables(5)；method(4)；improved(3)；transformation(3)；solid(3)；rocket(3)；accuracy(3)
...
- 原句：To improve perfor mance and ensure reliability, accounting for these uncertainties at every design stage is essential [2,3].
...
- 原句：Compared to numerical variables, quantifying correlations among field variables poses a substantially greater challenge.
  可迁移模板：Compared to numerical variables, quantifying correlations among field variables poses a substantially greater challenge.
- 原句：To address the challenge of incorporating correlations in UP, re searchers have carried out extensive investigations [12–14].
  可迁移模板：To address the challenge of incorporating correlations in METHOD, re searchers have carried out extensive investigations [X–X].
- 原句：Notably, the proposed method requires correlated field variables to have the same dimension.
...
- 原句：However, decoupling introduces a major * Corresponding author.
  可迁移模板：However, decoupling introduces a major * Corresponding author.
- 原句：However, existing uncertainty propagation (UP) methods for multidisciplinary systems often neglect correlations among field variables, leading to reduced accuracy.
  可迁移模板：However, existing uncertainty propagation (METHOD) methods for multidisciplinary systems often neglect correlations among field variables, leading to reduced accuracy.
- 原句：However, these studies primarily focus on correlations among input random variables within single-discipline UP, leaving multidisci plinary correlations largely unaddressed.
  可迁移模板：However, these studies primarily focus on correlations among input random variables within single-discipline METHOD, leaving multidisci plinary correlations largely unaddressed.
#### 方法提出句
- 原句：However, existing uncertainty propagation (UP) methods for multidisciplinary systems often neglect correlations among field variables, leading to reduced accuracy.
  可迁移模板：However, existing uncertainty propagation (METHOD) methods for multidisciplinary systems often neglect correlations among field variables, leading to reduced accuracy.
...
- 原句：These results demonstrate that the proposed method effectively addresses UP problems involving both numerical and field correlated variables, significantly enhancing the accuracy of UP.
  可迁移模板：These results demonstrate that the proposed METHOD problems involving both numerical and field correlated variables, significantly enhancing the accuracy of METHOD.
- 原句：Uncertainty-based multidisciplinary design optimization (UMDO) has emerged as a promising framework to systematically incorporate uncertainty into the design process [4–6].
  可迁移模板：Uncertainty-based multidisciplinary design optimization (METHOD) has emerged as a promising framework to systematically incorporate uncertainty into the design process [X–X].
#### 结果呈现句
- 原句：These results demonstrate that the proposed method effectively addresses UP problems involving both numerical and field correlated variables, significantly enhancing the accuracy of UP.
  可迁移模板：These results demonstrate that the proposed METHOD problems involving both numerical and field correlated variables, significantly enhancing the accuracy of METHOD.
- 原句：These results demonstrate that the proposed method effectively addresses UP problems involving both numerical and field correlated variables, significantly enhancing the accuracy of UP. limitation: correlations among state variables are usually neglected, leading to reduced accuracy in the independent UP process.
  可迁移模板：These results demonstrate that the proposed METHOD problems involving both numerical and field correlated variables, significantly enhancing the accuracy of METHOD. limitation: correlations among state variables are usually neglected, leading to reduced accuracy in the independent METHOD process.
...
- 原句：In the case study of UP across multiple disciplines of a solid rocket system, using Monte Carlo simulation (MCS) as the benchmark, incorporating variable correlations yields notable improve ments: the standard deviation accuracy of velocity and total energy at the first-stage separation point increased by 22.75 % and 32.57 %, respectively, while the accuracy of their lower bounds improved by 5.20 % and 4.20 %.
  可迁移模板：In the case study of METHOD across multiple disciplines of a solid rocket system, using Monte Carlo simulation (METHOD) as the benchmark, incorporating variable correlations yields notable improve ments: the standard deviation accuracy of velocity and total energy at the first-stage separation point increased by X and X, respectively, while the accuracy of their lower bounds improved by X and X.
- 原句：To improve perfor mance and ensure reliability, accounting for these uncertainties at every design stage is essential [2,3].
  可迁移模板：To improve perfor mance and ensure reliability, accounting for these uncertainties at every design stage is essential [X,X].
- 原句：This enables independent analysis within each discipline and aligns with practical engineering design practices.
  可迁移模板：This enables independent analysis within each discipline and aligns with practical engineering design practices.
- 原句：In the case study of UP across multiple disciplines of a solid rocket system, using Monte Carlo simulation (MCS) as the benchmark, incorporating variable correlations yields notable improve ments: the standard deviation accuracy of velocity and total energy at the first-stage separation point increased by 22.75 % and 32.57 %, respectively, while the accuracy of their lower bounds improved by 5.20 % and 4.20 %.
  可迁移模板：In the case study of METHOD across multiple disciplines of a solid rocket system, using Monte Carlo simulation (METHOD) as the benchmark, incorporating variable correlations yields notable improve ments: the standard deviation accuracy of velocity and total energy at the first-stage separation point increased by X and X, respectively, while the accuracy of their lower bounds improved by X and X.
- 原句：By integrating a warm-start-based maximum entropy approach into the Nataf transformation, the method enables efficient and accurate handling of field variables.
...
- 引文簇样例：[1], [2,3], [10], [11], [15], [16], [17], [21], [22,23], [27], [0, 10], [28]
